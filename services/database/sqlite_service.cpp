...
quark::MTSqliteService::MTSqliteService(const std::string& path)
{
    ...
    if (rc)
    {
        const auto stdErrMsg = std::format("Can't open database: {}", sqlite3_errmsg(db));
...
std::unique_ptr<quark::MTSqliteCommand> quark::MTSqliteService::createCommand(const std::string& sqlText)
{
    ...
    if (rc)
    {
        auto stdErrMsg = std::format("Can't prepare statement: {}", sqlite3_errmsg(this->sqlite3Database));
        throw MTException(stdErrMsg);
    }
...
std::string quark::MTSqliteService::sqliteVersion()
{
    ...
    if (verColumn == nullptr) throw MTException("Can't get sqlite version");
