...
void cSqliteSelectNames() {
  ...
  QKSqliteService *sqliteService = QKSqliteServiceCreate(dbPath, NULL);
  QKString *sqlText =
      QKStringCreate("SELECT 'hello呀哈哈' as strVal, 128 as intVal;");
  ...
  QKSqliteColumn *strColByName =
      QKSqliteRowGetColumnByName(sqlRow, strColName, NULL);
  QKString *strValByName = QKSQliteColumnGetStringValue(strColByName, NULL);

  QKSqliteColumn *intColByName =
      QKSqliteRowGetColumnByName(sqlRow, QKStringCreate("intVal"), NULL);
  int intValByName = QKSQliteColumnGetIntValue(intColByName, NULL);

  QKSqliteServiceDelete(sqliteService, NULL);
  printf("cSqliteSelectNames: %s, %d\n\t%s, %d\n", strValData, intVal,
         QKStringGetData(strValByName), intValByName);
}
...
void cSqliteStatParams() {
  ...
  QKSqliteCommand *sqlCmd =
      QKSqliteServiceCreateCommand(sqliteService, sqlText, NULL);
